...
For normal projects, start with the project-type setup: `typescript-bun` (the primary Bun workspaces monorepo stack: React, Vite, Hono, Drizzle, Vitest), `go-cli` (command-line tools without a TUI), `go-tui` (Bubble Tea v2 applications), or `rust-cli`. These project setups include the CONTEXT-driven spec workflow — `brainstorming`/`grill-with-docs` -> `write-idea` (product-level ideas) -> `write-prd` -> `write-techspec` -> `write-tasks` -> `implement-spec`/`implement-task` -> `qa-gate` -> `archive-spec` — with `domain-modeling` for `CONTEXT.md`/ADR capture and per-feature artifacts under `docs/specs/<slug>/`. Two workflow-only presets add a process to an existing repo: `context-workflow` (the spec workflow above) and `matt-pocock-workflow` (`ask-matt`/grill -> `to-spec` -> `to-tickets` -> `implement`, with `wayfinder` for large efforts). Add `release` when the project needs Cloudflare, Docker, and Sentry workflows, or `devops` for infrastructure repositories: Terraform, DigitalOcean, AWS, Cloudflare, and Docker, plus apply discipline, observability, and delivery evidence.
