...
```tsx
<motion.div layoutId={`row-${id}`} className="card">
  …
</motion.div>
...
- Material Design 3 — _Motion_ (physics, easing, durations)
- Apple Human Interface Guidelines — _Motion_
- Cassie Evans — _CSS animations & GSAP talks_ on premium feel
- Sarah Drasner — _SVG Animations_ (still the canonical text)
