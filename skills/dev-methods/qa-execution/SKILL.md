...
3. For each charter from Step 3, run the assigned tour against the surface:
   ...
   - Execute the tour's off-script actions, asking _"would this matter for this tour's theme?"_ at each one.
...
4. For every finding, capture the persona felt (not just the technical observation): _"a mobile user with one hand could not reach the submit button"_ is more actionable than _"button is 8px out of touch target"_.
